...
import org.jetbrains.plugins.template.ui.CallHierarchyToolWindow
import org.jetbrains.plugins.template.ui.CallHierarchyToolWindowFactory
...
class OrikaMappingTracerAction : AnAction() {
        ...
        ProgressManager.getInstance().run(object : Task.Backgroundable(project, "分析Orika映射调用链...", true) {
            override fun run(indicator: ProgressIndicator) {
                try {
                    ...
                    val simpleAnalyzer = project.getService(org.jetbrains.plugins.template.services.SimpleMappingAnalyzer::class.java)
                    
                    // 在ReadAction中进行PSI访问 - 使用简化分析器进行调试
                    val mappingRelations = ApplicationManager.getApplication().runReadAction<List<org.jetbrains.plugins.template.model.MappingRelation>> {
                    ...
                    val callChain = ApplicationManager.getApplication().runReadAction<List<org.jetbrains.plugins.template.model.MappingCall>> {
    ...
    private fun showResults(
        project: Project, 
        field: PsiField, 
        mappingRelations: List<org.jetbrains.plugins.template.model.MappingRelation>,
        callChain: List<org.jetbrains.plugins.template.model.MappingCall>
    ) {
        // 获取或创建工具窗口
